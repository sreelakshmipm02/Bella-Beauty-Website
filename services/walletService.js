...
export const recalculateOrderStatusFromItems = (items = []) => {
    ...
    if (statuses.every((status) => ["Returned", "Cancelled"].includes(status))) return "Returned";
    ...
    if (statuses.every((status) => completedStates.includes(status))) return "Delivered";
...
export const buildRefundAllocationMap = (order) => {
    ...
    const totalItemValuePaise = items.reduce((sum, item) => sum + toPaise(item.itemTotal || 0), 0);
    ...
    let allocatedPaise = provisionalShares.reduce((sum, share) => sum + share.paise, 0);
...
export const canRefundToWallet = (order) => {
    ...
    return Boolean(paymentStatus && !["Pending", "Failed"].includes(paymentStatus));
};

export const syncPaymentRefundStatus = (order) => {
    const refundedAmount = fromPaise(toPaise(order?.payment?.refundedAmount || 0));
    ...
    order.payment.status = refundedAmount >= totalPaid ? "Refunded" : "Partially Refunded";
};
...
export const creditWallet = async ({
    ...
    const updatedBalance = fromPaise(toPaise(user.wallet.balance || 0) + toPaise(normalizedAmount));
    const transaction = {
        type: "credit",
...
export const debitWallet = async ({
    ...
    const updatedBalance = fromPaise(toPaise(currentBalance) - toPaise(normalizedAmount));
    const transaction = {
        type: "debit",
        amount: normalizedAmount,
        balanceAfter: updatedBalance,
...
export const getWalletSnapshot = async (userId, transactionLimit = 8) => {
    ...
    return {
        ...
        transactions: transactionLimit > 0 ? transactions.slice(0, transactionLimit) : transactions
    };
};
