...
export const getAdminProductsList = async (page, limit, search, category, status) => {
    ...
    if (category && category !== 'all') matchQuery.categoryId = new mongoose.Types.ObjectId(category);
...
export const updateExistingProduct = async (productId, productData, variantsData, files) => {
    ...
    for (let i = 0; i < variantsData.length; i++) {
        ...
        if (files) {
            const newFilesForVariant = files.filter(f => f.fieldname === `variant_images_${i}`);
            if (newFilesForVariant.length > 0) {
                const newImageUrls = newFilesForVariant.map(file => file.path || file.secure_url || file.url);
        ...
        if (variant._id) {
            ...
            await ProductVariant.findByIdAndUpdate(variant._id, {
                ...
                attributes: variant.attributes, images: finalImages
            });
            ...
            const newVariant = await ProductVariant.create({
                productId: productId, sku: variant.sku, price: variant.price,
                stock: variant.stock, attributes: variant.attributes, images: finalImages
            });
