...
const normalizeOfferPayload = (payload) => {
    ...
    if (!offerName || !offerType || !targetId || !discountType || !discountValue || !maxDiscountValue || !startDate || !endDate) {
...
const ensureTargetExists = async (offerType, targetId) => {
    ...
    if (!target) {
        throw new AppError(`${offerType === "product" ? "Product" : "Category"} not found.`, 404);
    }
};
...
export const createOfferByAdmin = async (payload) => {
    ...
    await ensureTargetExists(normalizedPayload.offerType, normalizedPayload.targetId);
...
export const updateOfferById = async (offerId, payload) => {
    ...
    const offer = await Offer.findByIdAndUpdate(offerId, normalizedPayload, { new: true });
    if (!offer) throw new AppError("Offer not found.", 404);
    return offer;
};
