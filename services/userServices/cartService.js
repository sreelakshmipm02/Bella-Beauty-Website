...
export const addItemToCart = async (userId, variantId, quantity = 1) => {
  ...
  if (!(await variantIsActive(variant)))
  ...
  if (quantity > variant.stock)
    throw new AppError(`Only ${variant.stock} left in stock.`, 409);
  if (quantity > MAX_QTY_PER_ITEM)
    throw new AppError(`Maximum limit is ${MAX_QTY_PER_ITEM} per item.`, 400);
  ...
  if (existingItemIndex > -1) {
    ...
    if (newQty > variant.stock)
      throw new AppError(`Cannot add more. Only ${variant.stock} left.`, 409);
    if (newQty > MAX_QTY_PER_ITEM)
      throw new AppError(`Limit of ${MAX_QTY_PER_ITEM} reached.`, 400);
...
const buildAvailableCouponsForClient = async (
  grossSubtotal,
  ...
  return coupons.map((coupon) => {
    ...
    if (!isEligible) {
      message =
        grossSubtotal < (coupon.minOrderAmount || 0)
...
export const getCartData = async (userId) => {
    ...
    if (isProductActive && isVariantActive && isCategoryActive && hasStock) {
      ...
      formattedItems.push({
        ...
        offerDiscount: Number(
          (offerPricing.discountAmount * actualQty).toFixed(2),
        ),
        appliedOffer,
        outOfStock: false,
        variant: variant,
  ...
  if (cart.appliedCoupon?.couponId) {
    ...
    discountAmount = getEligibleCouponDiscount(
      coupon,
    ...
    if (coupon && discountAmount > 0) {
      ...
      couponChanged = true;
      adjustments.push(
        "Applied coupon was removed because it is no longer valid for this cart",
      );
    }
  }
...
export const updateItemQuantity = async (userId, variantId, newQuantity) => {
  ...
  if (newQuantity > variant.stock)
    throw new AppError(`Only ${variant.stock} left.`, 409);
  if (newQuantity > MAX_QTY_PER_ITEM)
    throw new AppError(`Limit is ${MAX_QTY_PER_ITEM}.`, 400);
  if (newQuantity < 1)
    throw new AppError("Quantity cannot be less than 1.", 400);
...
export const applyCouponToCart = async (userId, couponCode) => {
  ...
  if (cart.appliedCoupon?.code) {
    ...
    throw new AppError(
      "A coupon is already applied. Remove it before applying another one.",
      409,
    );
  }
