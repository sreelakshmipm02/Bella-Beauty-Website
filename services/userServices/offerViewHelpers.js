...
export const applyOffersToProductCards = (products, offers) => {
    return products.map((product) => {
        const activeVariants = (product.activeVariants || []).map((variant) => applyOfferToVariantView(variant, product, offers));
        const startingPrice = Math.min(...activeVariants.map((variant) => variant.price));
        const originalStartingPrice = Math.min(...activeVariants.map((variant) => variant.originalPrice));
        const bestVariant = activeVariants.find((variant) => variant.price === startingPrice) || activeVariants[0];
