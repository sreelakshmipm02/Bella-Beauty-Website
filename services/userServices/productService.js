...
import {
  applyOffersToProductCards,
  applyOfferToVariantView,
} from "./offerViewHelpers.js";
...
export const getShopData = async (queryFilters, skip, limit) => {
  ...
  if (category && mongoose.Types.ObjectId.isValid(category)) {
    pipeline.push({
      $match: { "categoryDetails._id": new mongoose.Types.ObjectId(category) },
    });
  }
    ...
    pipeline.push({
      $match: {
        $or: [
          {
            _id: {
              $in: productOfferIds.length ? productOfferIds : [unmatchedId],
            },
          },
          {
            "categoryDetails._id": {
              $in: categoryOfferIds.length ? categoryOfferIds : [unmatchedId],
            },
          },
        ],
      },
  ...
  if (search) {
    ...
    if (matchingOfferCategoryIds.length) {
      searchConditions.push({
        "categoryDetails._id": { $in: matchingOfferCategoryIds },
      });
    }
