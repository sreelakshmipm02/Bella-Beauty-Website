...
async function deleteGlobalAttribute(attributeId, buttonElement) {
    ...
    if (result.isConfirmed) {
        try {
            const response = await fetch(`/admin/attributes/${attributeId}`, { method: 'DELETE' });
            const data = await response.json();
...
function generateInternalAttrName() {
    ...
    document.getElementById('attrInternalName').value = label.toLowerCase().replace(/[^a-z0-9]/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '');
}
    ...
    try {
        ...
        if (data.success) {
            ...
            if (attr.possibleValues && attr.possibleValues.length > 0) {
                document.getElementById('editAttrPossibleValues').value = attr.possibleValues.join(', ');
            } else {
...
function generateEditInternalAttrName() {
    ...
    document.getElementById('editAttrInternalName').value = label.toLowerCase().replace(/[^a-z0-9]/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '');
}
...
async function submitCategoryForm(e) {
    ...
    try {
        ...
        if (data.success) {
            ...
            Swal.fire({
                title: 'Success!', text: 'Category added successfully.', icon: 'success', confirmButtonColor: '#e83e8c'
        ...
        submitBtn.innerHTML = '<span class="material-symbols-outlined text-[18px] mr-2">save</span> Save Category';
    }
}
...
async function submitEditCategoryForm(e) {
    ...
    try {
        ...
        if (data.success) {
            ...
            Swal.fire({
                title: 'Success!', text: 'Category updated successfully.', icon: 'success', confirmButtonColor: '#e83e8c'
            }).then(() => window.location.reload());
        } else {
            document.getElementById('editCategoryErrorText').textContent = data.message;
            document.getElementById('editCategoryError').classList.remove('hidden');
        }
    } catch (error) {
        document.getElementById('editCategoryErrorText').textContent = 'A network error occurred.';
        document.getElementById('editCategoryError').classList.remove('hidden');
    } finally {
        btn.disabled = false;
        btn.innerHTML = '<span class="material-symbols-outlined text-[18px] mr-2">save</span> Update Category';
    }
}
