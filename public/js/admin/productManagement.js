...
document
  ...
  .addEventListener("click", async function () {
    ...
    if (!window.currentVariantAttributes) {
      this.innerHTML =
        '<span class="material-icons-outlined animate-spin mr-1">progress_activity</span> Loading...';
      this.disabled = true;
      await fetchAndRenderAttributes(categoryId);
      this.innerHTML =
        '<span class="material-icons-outlined text-sm">add</span> Add New Variant';
      this.disabled = false;
    }
...
mainForm.addEventListener("submit", async function (e) {
  ...
  saveBtn.innerHTML =
    '<span class="material-icons-outlined animate-spin mr-2">progress_activity</span> Saving Product...';
  ...
  try {
    ...
    if (data.success) {
      ...
      Swal.fire(
        "Server Error",
        data.message || "Failed to save consolidated product.",
        "error",
      );
    }
  } catch (error) {
    ...
    Swal.fire(
      "Network Error",
      "A network error occurred while uploading product data.",
      "error",
    );
  } finally {
    saveBtn.disabled = false;
    saveBtn.innerHTML =
      '<span class="material-icons-outlined text-[20px]">save</span> Save Product';
  }
});
